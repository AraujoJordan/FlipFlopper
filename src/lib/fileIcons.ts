// Generated from material-extensions/vscode-material-icon-theme (MIT License).
// Source: https://github.com/material-extensions/vscode-material-icon-theme
// Icon SVG assets live in `public/icons` (see public/icons/LICENSE for the
// upstream copyright notice). Re-run `scripts/update-file-icons.md` steps to refresh.

/** File extension (may be multi-part, e.g. "d.ts", "spec.ts") -> icon name. */
...
/** Exact (lowercased) file name -> icon name. */
const FILE_NAME_ICONS: Record<string, string> = {
  ...
  ".dev.vars": "tune",
  ".devcontainer/devcontainer-lock.json": "3d",
  ".devcontainer/devcontainer.json": "3d",
  ".drone.yml": "drone",
  ".easignore": "expo",
  ".ecrc": "editorconfig",
  ...
  "bashly-settings.yaml": "bashly",
  "bashly-settings.yml": "bashly",
  ...
  "src/bashly-strings.yaml": "bashly-hook",
  "src/bashly-strings.yml": "bashly-hook",
...
/** Exact (lowercased) folder name -> icon name. */
const FOLDER_NAME_ICONS: Record<string, string> = {
  ...
  "__gitea/workflows__": "folder-gh-workflows",
  ...
  "__instruction__": "folder-meta",
  "__instructions__": "folder-meta",
  ...
  "__redis__": "folder-database",
  "__redis-db__": "folder-database",
  "__redislabs__": "folder-database",
  ...
  "_git": "folder-git",
  "_gitea": "folder-gitea",
  "_gitea/workflows": "folder-gh-workflows",
  ...
  "_instruction": "folder-meta",
  "_instructions": "folder-meta",
  ...
  "_redis": "folder-database",
  "_redis-db": "folder-database",
  "_redislabs": "folder-database",
  ...
  "-git": "folder-git",
  "-gitea": "folder-gitea",
  "-gitea/workflows": "folder-gh-workflows",
  "-githooks": "folder-git",
  ...
  "-input": "folder-input",
  "-inputs": "folder-input",
  "-instruction": "folder-meta",
  "-instructions": "folder-meta",
  ...
  "-redis": "folder-database",
  "-redis-db": "folder-database",
  "-redislabs": "folder-database",
  "-release": "folder-dist",
...
/** Progressively shorter dot-separated suffixes, longest first, so compound
 *  extensions like "spec.ts" are tried before the plain "ts" fallback.
 *  A leading dot (dotfiles like ".env") is not treated as a separator. */
function extensionCandidates(lowerName: string): string[] {
  const candidates: string[] = [];
  let idx = lowerName.indexOf(".");
  while (idx !== -1 && idx < lowerName.length - 1) {
    if (idx > 0) candidates.push(lowerName.slice(idx + 1));
    idx = lowerName.indexOf(".", idx + 1);
  }
  return candidates;
}

/** Resolve the material-icon-theme icon name for a file, falling back to the
 *  generic file icon. Exact file names win over extensions; longer compound
 *  extensions (e.g. "d.ts") win over shorter ones (e.g. "ts"). */
export function getFileIconName(filename: string): string {
  ...
  for (const candidate of extensionCandidates(lower)) {
    const byExt = FILE_EXTENSION_ICONS[candidate];
    if (byExt) return byExt;
  }
  return DEFAULT_FILE_ICON;
}

/** Resolve the material-icon-theme icon name for a folder by its basename. */
export function getFolderIconName(folderName: string, isRoot = false): string {
  if (isRoot) return ROOT_FOLDER_ICON;
  return FOLDER_NAME_ICONS[folderName.toLowerCase()] ?? DEFAULT_FOLDER_ICON;
}
